...
servers = [{name: 'Blackhole', url: 'http://blackhole.org', method: 'http'},
		{name: 'Secure Blackhole', url: 'https://blackhole.org', method: 'http'},
		{name: 'Pyload', url: 'http://192.168.0.2:8000/login', method: 'http'}]
...
SCHEDULER.every '30m', :first_in => 0 do |job|
	...
	servers.each do |server|
		if server[:method] == 'http'
			...
			if !failed and response.code == "200"
			 	result = 1
			 else
			 	result = 0
			 end
		...
		if result == 1
			...
			arrow = "fa fa-exclamation-circle"
			color = "red"
		end
